...
def find_dgs_chapter_in_toc(
    ...
    system_prompt = (
        "你是一个专业的年报目录分析助手。\n"
        "你的任务是严格从用户提供的【目录】文本中，找出与【董事、监事、高级管理人员】（简称DGS）最相关的那个章节。\n"
        "你需要提取该章节的【起始页码】、【结束页码】和完整的【章节标题】。\n"
        "规则：\n"
        "1. 优先寻找同时包含 '董事'、'监事' 和 '高级管理人员' 的章节。\n"
        "2. 如果找不到，寻找包含 '董事、监事' 或 '高管' 的章节。\n"
        "3. 章节标题必须是目录中的原文。\n"
        "4. 页码必须是目录中标记的数字。\n"
        "5. 如果目录中没有相关章节，或输入内容与目录完全无关，必须返回 -1, -1, ''。\n"
        "6. 你必须调用 `save_toc_analysis` 工具返回结果。"
    )
